<template>
  <div class='container'>
    ...
    <el-divider class='divide'>
      <i class='el-icon-document'>所有文章</i>
    </el-divider>
    <div v-if='posts' id='panel'>
      <!--getPosts()加载完成后才显示，因为用到了posts数据-->
      <div
      ...
      >
        ...
        <div class='post_body'>
          <vue-markdown class='post_summary' :source='items.summary'></vue-markdown>
        </div>
...
export default {
  ...
  methods: {
    ...
    handleCurrentChange (val) {
      // 改变页码
      const path =
        'post/getPosts?page=' + val + '&per_page=' + this.posts._meta.per_page // 在url中添加参数
      axios.get(path).then((response) => {
        console.log(response.data)
        this.posts = response.data
      })
    ...
    getPosts () {
      ...
      const path = 'post/getPosts?page=' + page + '&per_page=' + perPage // 在url中添加参数
      axios
        .get(path)
        .then((response) => {
          console.log(response.data)
